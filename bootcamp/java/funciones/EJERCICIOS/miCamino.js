...
for (let i = 0; i < miCamino.material.length; i++) {
  console.log(`lo que debo llevar es ${miCamino.material[i].nombre} y es ${miCamino.material[i].descripcion}`)
}
//renderizando
//miCamino.material.array.forEach(material, index => {
//  console.log()});
...
for (const cima of miCamino.cimas) {
  console.log(`para subir al ${cima.nombre} debo subir ${cima.altitud}mtrs y debo viajar a ${cima.localizacion}`)
}
//.filter es un bucle interno
//casos.filter((element) => element.nombre === "piolets")
...
let recorrido = miCamino.subiendoAlEverest[0][0].campoBase[0].campoI.campoII.campoIII.campoIV
const mensaje=[]
for (let i; i < recorrido.length; i++) {
  const elemento= recorrido[i]
